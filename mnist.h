...
#define TRAIN_IMAGE "src/train-images-idx3-ubyte"
#define TRAIN_LABEL "src/train-labels-idx1-ubyte"
#define TEST_IMAGE "src/t10k-images-idx3-ubyte"
#define TEST_LABEL "src/t10k-labels-idx1-ubyte"
...
void one_hot_label(int label[], int one_hot[][10], int num_data) {
	int i = 0;
	int j = 0;
	for (i=0; i<num_data; i++) {
		for (j=0; j<10; j++) {
...
void load_mnist()
{
	...
	/* Transform label to one-hot key */
	one_hot_label (train_label, one_hot_train_label, NUM_TRAIN);
	one_hot_label (test_label, one_hot_test_label, NUM_TEST);
}
